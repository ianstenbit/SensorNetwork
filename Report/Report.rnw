...
The graph coloring algorithm utilizes a smallest-last vertex ordering to perform coloring in linear time. The smallest-last vertex ordering is performed in linear time per the algorithm described in Matula, D.W, Beck, L.L 1983 \textsuperscript{[6]}. The algorithm creates buckets based on vertex degree and, in a linear pass through the vertices, puts each vertex in the bucket which represents the degree of the vertex. Subsequently, the first element from the smallest bucket is extracted until all buckets are empty. Each time a vertex is removed, all vertices adjacent to that vertex are moved from their current buckets to the bucket with a degree one lower than their previous degree. The terminal clique is found when all remaining vertices are in the same bucket, and the number of vertices in the bucket is equal to the degree of the bucket plus one.

\subsubsection{Graph Coloring}
Once the smallest-last vertex ordering has been computed, the coloring is done by iterating through the smalles-last vertex ordering and, at each point, assigning the first available color to the point on the graph by looking at the colors of points adjacent to the point in question. This is a non-heuristic graph coloring algorithm and is guaranteed to find a minimal number of colors on the graph \textsuperscript{[6]}. Verification of the linear-time claim for this algorithm can be found in the upcoming \textit{Algorithm Engineering} section.
...
\subsubsection{Smallest-Last Ordering}
...
\textit{Current Ordering: \{12, 17, 3\}}
\end{center}
...
\textit{Current Ordering: \{12, 17, 3, 18\}}
\end{center}
...
\textit{Current Ordering: \{12, 17, 3, 18, 5\}}
\end{center}
...
\textit{Current Ordering: \{12, 17, 3, 18, 5, 1\}}
\end{center}
...
\textit{Current Ordering: \{12, 17, 3, 18, 5, 1, 9\}}
\end{center}
...
\textit{Current Ordering: \{12, 17, 3, 18, 5, 1, 9, 6\}}
\end{center}
...
\includegraphics[width=5cm, height=5cm]{verification2}
\includegraphics[width=5cm, height=5cm]{verification3}
\includegraphics[width=5cm, height=5cm]{verification4}

\includegraphics[width=5cm, height=5cm]{verification5}
\includegraphics[width=5cm, height=5cm]{verification6}
\includegraphics[width=5cm, height=5cm]{verification7}

\begin{center}
\textit{Figure 2.5 - First 6 iterations of coloring}
\end{center}

\subsubsection{Backbone Selection}
After the colors have been identified, the top 4 colors are isolated and 6 possible backbones are created. The size of the giant component is then taken, and, in this case, the largest giant component is of cardinality 6. The backbone, therefore, is comprised of 6 nodes. Figure 2.6 shows the subgraph which contains the bes t backbone that was found. Note that the component of the subgraph on the right side of the graph is not part of the backbone, as it is not part of the giant component. The coverage of this backbone is 0.8, as 16 of the 20 nodes are reachable from the backbone.

\begin{center}
\includegraphics[width=8cm, height=8cm]{verification8}

\textit{Figure 2.6 - A backbone for the verification network (note that only the left component of the graph is the actual backbone)}
\end{center}

\subsection{Algorithm Efficacy}

Table 2.1 shows the coverages of the backbones computed for each of the 10 benchmarks (each of which will be explored more in the upcoming sections). Clearly, the algorithm performs quite well, as the coverages of the computed backbones are exceptionally high (the lowest is 0.996, or 99.6\% coverage). Furthermore, all algorithms run in linear time, and the use of the Processing language allows for real-time visualization of the graphs built by the algorithm. This real-time rendering is interactive, and contains multiple view modes for looking at individual colors and backbones. The efficiency of the algorithm was evident in the run-time graphs shown in the \textit{Algorithm Engineering} section, and the implementation in Python was even able to compute backbones for a graph of 2 million vertices with an average degree of 250 in a unit square distribution in only a few minutes.
...
<<bench1, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp1.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench1b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col1.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench1c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
R (Radius) & 0.101\\
$|E|$ (Number of Edges) & 29352\\
$Degree_{min}$ & 8\\
$Degree_{max}$ & 49\\
$Degree_{del,max}$ & 21\\
Number of Colors & 23\\
Max Color Class Size & 78\\
Terminal Clique Size & 20\\
$|V|_{largestBackbone}$ & 144\\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench1b}

\raisebox{1.75\height}{
\begin{tabular}{|l | l |}
\hline
\multicolumn{2}{|c|}{Backbone 1} \\
\hline
Vertices & 148 \\ 
Edges & 173 \\
Coverage & 0.99 \\ 
\hline
\multicolumn{2}{|c|}{Backbone 2} \\
\hline
Vertices & 148\\
Edges & 180\\
Coverage & 0.987\\
...
\includegraphics[width=12cm]{Report-bench1}

\includegraphics[width=7cm, height=7cm]{bench1}
\includegraphics[width=7cm, height=7cm]{report-bench1c}

\textit{The distribution of points/degrees and a backbone for benchmark 1}

\end{center}

\subsection{Benchmark 2}
<<bench2, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp2.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench2b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col2.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench2c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
\includegraphics[width=7cm, height=7cm]{Report-bench2b}
...
\includegraphics[width=12cm]{Report-bench2}

\includegraphics[width=7cm, height=7cm]{bench2}
\includegraphics[width=7cm, height=7cm]{report-bench2c}

\textit{The distribution of points/degrees and a backbone for benchmark 2}

\end{center}


\subsection{Benchmark 3}
<<bench3, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp3.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench3b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col3.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench3c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
Terminal Clique Size & 35\\
$|V|_{largestBackbone}$ & 1271\\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench3b}
...
\includegraphics[width=12cm]{Report-bench3}

\includegraphics[width=7cm, height=7cm]{bench3}
\includegraphics[width=7cm, height=7cm]{report-bench3c}

\textit{The distribution of points and a backbone for benchmark 3}

\end{center}

\subsection{Benchmark 4}
<<bench4, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp4.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench4b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col4.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench4c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
Terminal Clique Size & 31\\
$|V|_{largestBackbone}$ & 5040\\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench4b}
...
\includegraphics[width=12cm]{Report-bench4}

\includegraphics[width=7cm, height=7cm]{report-bench4c}

\textit{The distribution of points and a backbone for benchmark 4 is not shown as a graph with so many points is not readily visualized. }

\end{center}


\subsection{Benchmark 5}
<<bench5, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp5.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench5b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col5.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench5c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
Terminal Clique Size & 61\\
$|V|_{largestBackbone}$ & 2734\\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench5b}
...
\includegraphics[width=12cm]{Report-bench5}

\includegraphics[width=7cm, height=7cm]{report-bench5c}

\textit{The distribution of points and a backbone for benchmark 5 is not shown as a graph with so many points is not readily visualized. }

\end{center}

\subsection{Benchmark 6}
<<bench6, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp6.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench6b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col6.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench6c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
\includegraphics[width=7cm, height=7cm]{Report-bench6b}
...
\includegraphics[width=12cm]{Report-bench6}

\includegraphics[width=7cm, height=7cm]{bench6}
\includegraphics[width=7cm, height=7cm]{report-bench6c}

\textit{The distribution of points/degrees and a backbone for benchmark 6}

\end{center}

\subsection{Benchmark 7}
<<bench7, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp7.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench7b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col7.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench7c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
Terminal Clique Size & 57\\
$|V|_{largestBackbone}$ & 176\\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench7b}
...
\includegraphics[width=12cm]{Report-bench7}
 
\includegraphics[width=7cm, height=7cm]{bench7}
\includegraphics[width=7cm, height=7cm]{report-bench7c}

\textit{The distribution of points and a backbone for benchmark 7}
 
\end{center}

\subsection{Benchmark 8}
<<bench8, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp8.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench8b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col8.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench8c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
Terminal Clique Size & 33\\
$|V|_{largestBackbone}$ & 321\\
Faces on Largest Backbone & 101 \\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench8b}
...
\includegraphics[width=12cm]{Report-bench8}

\includegraphics[width=7cm, height=7cm]{bench8}
\includegraphics[width=7cm, height=7cm]{report-bench8c}

\textit{The distribution of points/degrees and a backbone for benchmark 8}

\end{center}

\subsection{Benchmark 9}
<<bench9, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp9.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench9b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col9.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench9c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
\includegraphics[width=7cm, height=7cm]{Report-bench9b}
...
\includegraphics[width=12cm]{Report-bench9}

\includegraphics[width=7cm, height=7cm]{bench9}
\includegraphics[width=7cm, height=7cm]{report-bench9c}

\textit{The distribution of points/degrees and a backbone for benchmark 9}

\end{center}

\subsection{Benchmark 10}
<<bench10, fig=TRUE, echo=FALSE, eps=FALSE, width=8, height=5,include=FALSE>>= 
source("data/tmp10.txt")
ts.plot(ts(lens), ts(delLens), gpars= list(main="Vertex Degrees Originally and at Deletion",col=rainbow(2), xlab="Vertex",ylab="Degree"))
legend('bottom', c("Original", "Deleted") , bty='n',lwd=4, lty=20, col=rainbow(2), cex=1)
@
<<bench10b, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
source("data/col10.txt")
barplot(cols, main="Number of Vertices in each Color", xlab="Color", ylab="Number of Vertices")
@
<<bench10c, fig=TRUE, echo=FALSE, eps=FALSE, width=4, height=4,include=FALSE>>= 
hist(lens, main="Histogram of Vertex Degree", xlab="Degree", ylab="Number of Vertices")
@
...
Terminal Clique Size & 50\\
$|V|_{largestBackbone}$ & 3758\\
\hline
\end{tabular}
}
\includegraphics[width=7cm, height=7cm]{Report-bench10b}
...
\includegraphics[width=12cm]{Report-bench10}

\includegraphics[width=7cm, height=7cm]{report-bench10c}

\textit{The distribution of points and a backbone for benchmark 10 is not shown as a graph with so many points is not readily visualized. }

\end{center}


\subsection{Real-Time Graphics Interface}
The graphs generated in this project can be visualized interactivelly using Processing. While rendering the sphere, one can drag the sphere around to see it from different perspectives, and press number keys to switch between view modes (available options include seeing the whole graph, seeing individual backbones, or seeing individual colors).

The code for the entire project, including the real-time visualization, is available at:
\begin{center}
\textit{https://www.github.com/ianjjohnson/SensorNetwork}
\end{center}


%EACH:
% - plot original degree and deleted degree of each node
% - plot sizes of each color (num nodes of that color)
% - table of ID, N, R, |E|, min degree, avg degree, max degree, max degree when deleted, number of colors, max color class size, terminal clique size, and number of edges in largest backbone, and faces (for spheres)
